...
class BCQ(object):
	def __init__(self, args, lmbda=0.75, phi=0.05):
		...
		self.actor = Actor(args.state_dim+args.h_state_dim, args.action_dim, phi).to(device)
	...
	def train(self, replay_buffer, batch_size=256,iterations=1):
		...
		for it in range(iterations):
			...
			if not self.args.true_reward:
				...
				reward = process_reward(h_response, self.args.reward_type)
			...
			# current_Q1, current_Q2 = self.critic(state, action)
			# u1=target_Q-current_Q1
			# u2=target_Q-current_Q2
			# critic_loss = 2*(torch.abs(self.expectile-torch.le(u1,0).float())*(u1**2)).mean()+2*(torch.abs(self.expectile-torch.le(u2,0).float())*(u2**2)).mean()
